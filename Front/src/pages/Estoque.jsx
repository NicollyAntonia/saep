...
export default function Estoque() {
  ...
  async function movimentar() {
    ...
    const ferramenta = produtos.find(p => p.id === Number(mov.ferramenta_id));

    if (!ferramenta) {
      return alert("Ferramenta inválida.");
    }

    if (mov.tipo === "saida" && mov.quantidade > ferramenta.quantidade) {
      return alert(
        `Quantidade indisponível! Estoque atual: ${ferramenta.quantidade}.`
      );
    }

    try {
      await api.post("/movimentacoes/criar/", {
        ferramenta: Number(mov.ferramenta_id),
        tipo: mov.tipo,
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {movs.length > 0 ? (
                movs.map((m) => {
                  const ferr = produtos.find(p => p.id === m.ferramenta);
                  return (
                    <tr key={m.id} className="border-b border-purple-600">
                      <td className="p-3">{m.id}</td>
                      <td className="p-3">{ferr ? ferr.nome : m.ferramenta}</td>
                      <td
